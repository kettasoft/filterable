...
The engine preserves group boundaries when it generates nested `where` and `orWhere` clauses. Each group owns the boolean used to join its direct children, including when groups are nested recursively.
...
Permissive mode can omit rejected leaves while preserving the remaining conditions in their original group. Malformed tree structure may still make the request unusable, so validate the incoming JSON shape before filtering.
